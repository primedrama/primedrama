...
async function searchTMDB(q) {
  ...
  const data = await fetchJSON(`${BASE}/search/multi?api_key=${API_KEY}&query=${encodeURIComponent(q)}`);
  el.innerHTML = "";
  ...
  if (data?.results?.length) {
    latestSearches.unshift({ title: q, poster_path: data.results[0].poster_path });
    latestSearches = latestSearches.slice(0, 10); // keep max 10
    displayLatestSearches();
  }
}

function displayLatestSearches() {
  const container = document.getElementById("latest-searches");
  if (!container) return;
  container.innerHTML = "";
  latestSearches.forEach(item => {
    const card = document.createElement("div");
    card.className = "card";
